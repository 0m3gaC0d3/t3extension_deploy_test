...
read -r -d '' HELP <<EOF
t3extension_deploy_test test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/t3extension_deploy_test/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/t3extension_deploy_test/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/t3extension_deploy_test/Tests/Unit"
            ;;
    esac
fi
